    ...
    const walk = (node: Element) => {
      for (const child of Array.from(node.childNodes)) {
        ...
        if (attr(c,'begin') && !nestedTimed) {
          ...
          const hasEnd=Boolean(attr(c,'end')||attr(c,'dur'));
          words.push({text:value,start,end:attr(c,'end')?stamp(attr(c,'end')!):attr(c,'dur')?start+stamp(attr(c,'dur')!):start,joinNext:true,...(hasEnd?{}:{endEstimated:true})});
          text += value;
        } else walk(c);
      }
    };
