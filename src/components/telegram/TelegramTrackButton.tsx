...
import { Send, Download, Loader2, Check, X } from 'lucide-react';
    ...
    {open && <div className="telegram-transfer-backdrop" role="dialog" aria-modal="true" aria-label="Добавить музыку в Telegram-профиль">
      <section className="telegram-transfer-panel">
        ...
        {sent ? <p className="transfer-success"><Check size={18}/> Трек добавлен в ваш Telegram-профиль.</p> : <>
          ...
          <button className="primary-button" disabled={busy || track.access !== 'playable'} onClick={transfer}>{busy ? <Loader2 size={16} className="animate-spin"/> : <Send size={16}/>} {busy ? 'Загружаем и добавляем…' : 'Добавить в профиль'}</button>
